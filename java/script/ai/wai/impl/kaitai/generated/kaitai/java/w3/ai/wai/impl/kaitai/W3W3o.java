...
public class W3W3o extends KaitaiStruct {
    ...
    private void _read() {
        ...
        if (useItem() == 1) {
            this.item = new W3W3u(this._io);
        }
        this.useDestructable = this._io.readU4le();
        if (useDestructable() == 1) {
            this.destructable = new W3W3u(this._io);
        }
        this.useDoodad = this._io.readU4le();
        if (useDoodad() == 1) {
            this.doodad = new W3W3a(this._io);
        }
        ...
        if (useBuff() == 1) {
            this.buff = new W3W3u(this._io);
        }
        this.useUpgrade = this._io.readU4le();
        if (useUpgrade() == 1) {
            this.upgrade = new W3W3a(this._io);
        }
    }
