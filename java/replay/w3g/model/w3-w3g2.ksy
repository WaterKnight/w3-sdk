...
seq:
  ...
  - id: records_header
    type: records_header
  - id: record
    type: record_chunk(records_header.record_size, records_header.num_record)
    size: records_header.record_size * records_header.num_record
  - id: random_seed
    type: u4
  - id: select_mode
    type: u1
...
types:
  ...
  records_header:
    ...
    instances:
      record_size:
        value: (num_start_bytes - 4 - 3) / num_record
  record_chunk:
    params:
      - id: record_size
        type: u2
  ...
  player_or_0x19_loop:
    ...
    seq:
      ...
      - id: next
        type: player_or_0x19_loop(discriminator)
        if: discriminator == 0x16
  ...
  player:
    ...
    seq:
      ...
      - id: fill
        type: u2
        repeat: expr
        repeat-expr: num_fill
        #if: num_fill != 0
        #custom_or_ladder
        if: custom_or_ladder != 0x08
      - id: runtime
        type: u4
        if: custom_or_ladder == 0x08
      - id: id_race_flag
